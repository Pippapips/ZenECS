...
namespace ZenECS.Core.Systems
{
    ...
    /// <summary>
    /// Variable-step frame logic (non-deterministic).
    /// <para>
    /// 실행 시점:
    /// - BeginFrame 또는 LateFrame 단계에서 호출될 수 있으며,
    ///   카메라 로직(FrameView), UI/HUD(FrameUI), 디버그 뷰 등
    ///   “보여주기/제어” 관련 비결정론적 로직에 적합합니다.
    /// </para>
    /// <para>
    /// 특징:
    /// - dt는 가변 값(프레임 시간)입니다.
    /// - World의 결정론 상태(FixedPosition2D, FixedRotation2D, Health 등)는
    ///   읽기 전용으로 사용하는 것을 권장하고,
    ///   뷰 전용 컴포넌트나 Unity 오브젝트, UI 상태만 변경하는 것이 이상적입니다.
    /// </para>
    /// </summary>
