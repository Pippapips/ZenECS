...
namespace ZenECS.Adapter.Unity.Infrastructure
{
    public static class WatchQueryRunner
    {
        ...
        public static bool TryCollectByWatch(object system, IWorld w, List<Entity> outList)
        {
            ...
            var hasGeneric = w.GetType().GetMethod("HasComponent", BindingFlags.Public | BindingFlags.Instance);
        ...
        /// <summary>[Watch]의 AllOf 컴포넌트를 모두 가진 엔티티를 수집(항상 동작)</summary>
        public static int TryCountByWatch(object system, IWorld w)
        {
            var attrs = system.GetType().GetCustomAttributes(typeof(ZenSystemWatchAttribute), false)
                .Cast<ZenSystemWatchAttribute>().ToArray();
